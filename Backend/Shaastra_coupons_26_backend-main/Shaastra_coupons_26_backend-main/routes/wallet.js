...
// ============================================
// POST /api/wallet/send-bulk-csv
// Only for Finance Core
// ============================================
router.post('/send-bulk-csv', [authMiddleware, isFinanceCore, transactionLimiter], async (req, res) => {
  try {
    const { recipients, sPin } = req.body;
    // recipients format: [{ userId: 'USER1', amount: 100 }, ...]

    if (!recipients || !Array.isArray(recipients) || recipients.length === 0) {
      return res.status(400).json({ message: 'Invalid or empty recipients list.' });
    }

    if (!sPin) {
      return res.status(400).json({ message: 'S-Pin is required.' });
    }
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      const totalAmountToSend = recipients.reduce((acc, r) => acc + Number(r.amount), 0);

      if (!sender || sender.balance < totalAmountToSend) {
        throw new Error(`Insufficient balance. You need ₹${totalAmountToSend.toFixed(2)} but have ₹${sender.balance.toFixed(2)}.`);
      }

      // Fetch all target users in one query
      const receiverUserIds = recipients.map(r => r.userId.toUpperCase().trim());
      const receivers = await User.findAll({
        where: { userId: { [Op.in]: receiverUserIds } },
        transaction: t
      });

      // Map for quick lookup
      const receiverMap = new Map();
      receivers.forEach(r => receiverMap.set(r.userId, r));

      // Check for missing users
      const foundIds = new Set(receivers.map(r => r.userId));
      const missingIds = receiverUserIds.filter(id => !foundIds.has(id));

      if (missingIds.length > 0) {
        throw new Error(`The following User IDs were not found: ${missingIds.join(', ')}`);
      }
      ...
      for (const recipientData of recipients) {
        const targetUserId = recipientData.userId.toUpperCase().trim();
        const amount = Number(recipientData.amount);
        const receiver = receiverMap.get(targetUserId);
      ...
      createdTransactions.forEach((newTxn) => {
         if (io) {
             io.to(newTxn.receiverUserId).emit("transaction_received", {
                 id: newTxn.id,
    ...
    res.status(500).json({ message: error.message || 'Server error during bulk transfer.' });
  }
});
